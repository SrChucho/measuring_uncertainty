% -------------------------------------------------------------------------
% Plot Figure 2 in the Paper (IRFs under Minimal Constraints)
% -------------------------------------------------------------------------
...
% 3 specifications 
% ind_spec == 1: covariance restriction only -- stored in IRF_cov.mat (No constraints, use all 1.5 million)
% ind_spec == 2: covariance + external variable 
% ind_spec == 3: covariance + external variable + gbar4 to gbar6 
...
if ind_spec > 1
% Generate B
B_out= gen_B_minimal(vec_A, QQ, X_var, X_exp,const_var,data,dates_lag,lag,ind_spec);
% Gen IRF
[lo,hi]=gen_lo_hi(vec_A, B_out,X_var, X_exp,const_var,data,lag);

else % load pre-saved IRF for all 1.5 million rotations. 
    load IRF_cov 
end
