function fig = plot_irf_fan_CI(Hi,Lo,lim,ylab)
fig=figure;

% Load CI Results
load MC_allsol.mat
Hi(:,:,2) = hi_CI; %With constraints
Lo(:,:,2) = lo_CI; %With constraints
Hi(:,:,3) = hi_CI_noconst; %Without constraints
Lo(:,:,3) = lo_CI_noconst; %Without constraints
...
for i=1:3;
    for j=1:3;
        ...
        if i == 2 & j == 2
            leg = legend('Identified Set','95% CI');
