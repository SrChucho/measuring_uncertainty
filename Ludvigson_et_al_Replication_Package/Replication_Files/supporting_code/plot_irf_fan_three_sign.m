function fig = plot_irf_fan_three_sign(Hi,Lo,lim,ylab)

% Plot impulse responses for Figure 2
...
for i=1:3;
    for j=1:3;
        ...
        hold on
        ciplot_h( Lo(k,:,1)*scale,Hi(k,:,1)*scale,[0:lim],[ 0.6843 0.8157 0.9182],1);
        hold on
        plot([0:lim],  Lo(k,:,2)*scale,'k--','linewidth',2)
        hold on
        plot([0:lim], Hi(k,:,3)*scale,'r-.','linewidth',1)
        hold on
        plot([0:lim],  Hi(k,:,2)*scale,'k--','linewidth',2)
        hold on
        plot([0:lim],  Lo(k,:,3)*scale,'r-.','linewidth',2)
        ...
        if i == 3 & j == 3
            ...
            leg = legend('$\bar{g}_{Z}= 0$','$\bar{g}_{Z}= 0$, $\bar{g}_{C}\geq 0$',...
                '$\bar{g}_{Z} = 0$, $\bar{g}_{C}\geq 0$, $\bar{g}_{E4}$ - $\bar{g}_{E6}$');
            
            set(leg,'Location','southwest','box','off','Fontsize',8.5,'fontweight','bold','interpreter','latex')
        else
